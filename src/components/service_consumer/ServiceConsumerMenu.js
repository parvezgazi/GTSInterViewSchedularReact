...
class ServiceConsumerMenu extends Component {
  ...
  componentDidMount(){
    ...
    axios.get(personalURL,{ headers: {'Auth_Token' : `Bearer ${token}`} })
   .then(userResponse => {
    ls.set('userPersonalDetails', userResponse.data)
    this.setState({
      user_name : userResponse.data.gts_user_first_name + " "+userResponse.data.gts_user_last_name
    })
      ...
      <div>
       <div className="container-fluid">
         <InputGroup>
           ...
           <h6 style={{paddingBottom:"20px"}}>&nbsp;<label style={{color:'black',fontSize:13}}><b>Name:</b></label><label style={{color:'red',fontSize:13 }}>{this.state.user_name}</label>
